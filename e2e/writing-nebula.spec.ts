 ...
 * Articles cluster by type (typed posts schema: investigation / essay /
 * failure-story / build-log); the list view is always the first-class accessible
 * equivalent (backlog rule: never force spatial navigation for essential discovery).
 *
 * Adapted from the pre-adoption main line (commit 3655f816) to the cloud
 * baseline's typed-posts schema:
 *   - type field: investigation / essay / failure-story / build-log
 *   - tags field (was: topics) for subject-matter keywords
 *   - shelves structure in the list panel
 *
...
test.describe('writing nebula map — EXP-027', () => {
  ...
  test('every article in the list view is reachable in the map view (list equivalence)', async ({ page }) => {
    ...
    // List rows carry data-entry-type — the attribute for typed article links.
    ...
    // Map cards are located by their class; classification in the map is on the
  ...
  // -------------------------------------------------------------------------
  // Map view metadata — readable without HUD decoding
  // -------------------------------------------------------------------------
  ...
  test('map view exposes readable metadata for each article card', async ({ page }) => {
    ...
    // Subject-matter topics (tags field in cloud schema: java, jvm, debugging, production).
  ...
  test('articles cluster by editorial type in the map view', async ({ page }) => {
    ...
    // The investigation cluster is present (memory-leak post is type=investigation).
    const cluster = page.locator('.nebula-cluster[data-cluster-type="investigation"]')
    await expect(cluster).toBeVisible()

    // It has a plain-language heading (SHELF_LABEL['investigation'] = 'Investigations').
