...
type: investigation
draft: false
# EVIDENCE (P13) — the artifacts backing this investigation, rendered as an
# aside by the post layout. Only what the article itself already references
# lives here; the owner's profiler captures (flamegraph, retained-memory
# table) join this list when supplied (docs/INPUTS-NEEDED.md #7).
evidence:
  - label: Original write-up (in French) — Takima on Medium, December 2023
    url: https://medium.com/takima/memory-leak-search-and-destroy-9e70c5785cfb
  - label: The sentry-java bug we hit — upstream issue #2496
    url: https://github.com/getsentry/sentry-java/issues/2496
  - label: The upstream fix, shipped in SDK 6.13 — PR #2505
    url: https://github.com/getsentry/sentry-java/pull/2505
# FORMALIZED CO-AUTHORSHIP (P6): both names flow into the BlogPosting JSON-LD
# author array (BaseHead) and the visible byline on the article header. The
# provenance block replaces the old italic first line of the body — the article
# page and the /writing index both render it from this data.
authors: ["Iliès Beldjilali", "Lansana Diomande"]
provenance:
  note: First published in French on Takima's Medium publication, December 2023
  url: https://medium.com/takima/memory-leak-search-and-destroy-9e70c5785cfb
---
