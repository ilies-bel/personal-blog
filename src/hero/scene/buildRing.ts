...
import { AdditiveBlending, BufferAttribute, BufferGeometry, Points, Scene, ShaderMaterial, Sphere, Vector3 } from 'three';
...
export interface RingRig extends UniformRig {
  pts: Points; // the ring band (frame toggles .visible)
  geo: BufferGeometry;
  mat: ShaderMaterial;
  uniforms: Uniforms;
  dispose: () => void;
}
export function buildRing(scene: Scene, pixelRatio: number): RingRig {
  const ringN = 64000;
  ...
  geo.setAttribute('position', new BufferAttribute(new Float32Array(ringN * 3), 3));
  geo.setAttribute('aAng', new BufferAttribute(ringAng, 1));
  geo.setAttribute('aSeed', new BufferAttribute(ringSeed, 1));
