...
export const diskVertexShader = /* glsl */ `
  ...
  void main(){
    ...
    if(uGiant > 0.0){
      ...
      // The red giant is the DEFAULT whenever the cloud isn't explicitly yellow /
      // nebula / dot. NOTE: uNebula is held high across the WHOLE nebula→star collapse
      // handoff — the real nebula, the collapse window AND its floor crossfade band
      // (lifecycle holds the collapse geometry simBlend>0 there via inWindowGeo, so
      // nebulaShader → uNebula stays 1) — so the cloud reads as converging nebula GAS
      // through the handoff and this default stays 0 there. Without that hold, the floor
      // crossfade (where simBlend/uNebula used to snap to 0 yet the body-ownership ladder
      // still kept the cloud briefly visible UNDER the forming yellow mesh) flashed a
      // full red giant for a few eased frames.
