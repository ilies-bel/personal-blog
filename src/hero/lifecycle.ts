...
export function lifecycle(input: LifecycleInput): StarState {
  ...
  if (sunWindow) {
    ...
    // (nebula snaps at 3.5) so the handoff from yellow isn't a hard pop. Keep bloom
    // moderate and exposure DOWN so the blue→red ramp survives; push saturation UP.
    const ne = smoothstep01((stage - 3.5) / 0.35);
    bloomStrength = bloomStrength * (1 - ne) + 0.3 * ne; // soft strand glow
    bloomRadius = cfg.bloomRad * (1 - ne) + 0.7 * ne; // wide soft halo
    exposure = exposure * (1 - ne) + 0.5 * ne;
    olive *= 1 - ne; // no olive cast
    warmth *= 1 - ne; // no warm cast (let blue/cyan show)
    gradeSat = gradeSat * (1 - ne) + 1.45 * ne; // vivid Crab palette
    diskSat = diskSat * (1 - ne) + 1.35 * ne;
  } else {
    bloomRadius = cfg.bloomRad;
  }
  ...
  if (!reduced) {
    ...
    // spill past the edges — we sit close inside the cloud. Ease in as the nebula
    // arrives (3.1 → 3.7), hold close, then pull back OUT toward the pale blue dot
