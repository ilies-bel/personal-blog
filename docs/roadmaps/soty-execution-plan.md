...
1. Continue P11→P14 sequentially (one subagent per phase, phases share files — never parallel in one tree), pushing after each phase. NO rebase onto origin/main; no PR.
2. P11 notes from P10: `docs/ANALYTICS.md` stub should include the web-vitals RUM wiring the perf-report/behind-the-build prose now points at (field CWV pending RUM); knip export-level findings are still warn-level — P11 owns tightening them; when a git remote lands, verify the new `lighthouse` ci.yml job on a real runner and consider promoting LCP/TBT to errors there (acceptance-gates.md documents the promotion); regenerating perf-report.json is a release step: `pnpm build && node scripts/gen-perf-report.mjs && pnpm build` (second build bakes the fresh JSON into the page).
