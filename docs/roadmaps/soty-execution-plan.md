...
1. Continue P12→P14 sequentially (one subagent per phase, phases share files — never parallel in one tree), pushing after each phase. NO rebase onto origin/main; no PR.
2. P12 notes from P11: any new inline script (OG-card wiring must NOT add one — keep scripts bundled) changes the CSP hashes and trips gate 13 (`gen-csp-hashes.mjs --check`); regenerate + commit the doc as part of the phase, and remember the live Cloudflare rule needs re-pasting (owner action). When the remote lands: verify prod-crawl.yml + the lighthouse job on real runners; after the owner applies the header rules, flip `WARN_ONLY_HEADERS` to 'false' in prod-crawl.yml. Release steps now live in docs/RUNBOOK.md — P12's capture tooling feeds its archive procedure. Regenerating perf-report.json remains a release step: `pnpm build && node scripts/gen-perf-report.mjs && pnpm build`.
3. Carried P9 notes: the marker DIVE (home → article) still runs its authored 1.5s cinematic (deliberately exempt as a hero set-piece; revisit if the jury reads it as a route transition); the graveyard/behind-the-build live ArticleScene boots slowly on software renderers (scene:ready can exceed 45s) — keep capture scripts' long timeouts.
