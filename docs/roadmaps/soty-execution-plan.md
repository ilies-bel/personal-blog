...
1. Continue P10→P14 sequentially (one subagent per phase, phases share files — never parallel in one tree), pushing after each phase. NO rebase onto origin/main; no PR.
2. P10 notes from P9: the marker DIVE (home → article) still runs its authored 1.5s cinematic (deliberately exempt as a hero set-piece; revisit if the jury reads it as a route transition); budgets.json should also cap the grain data-URI/backdrop CSS growth; the graveyard/behind-the-build live ArticleScene boots slowly on software renderers (scene:ready can exceed 45s) — keep capture scripts' long timeouts.
