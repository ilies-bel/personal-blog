...
1. Continue P13→P14 sequentially (one subagent per phase, phases share files — never parallel in one tree), pushing after each phase. NO rebase onto origin/main; no PR.
2. P13 notes from P12: the OG cards bake page h1 copy into `scripts/gen-og-cards.mjs`'s CARDS table (title strings) — if P13's editorial pass rewrites a page h1, update the card title there and regenerate `public/og/` in the same commit (gate 14 won't catch copy drift, only broken/missized cards). Post cards derive from frontmatter titles automatically. When P13 restructures /projects around problem/role/outcome, consider whether the projects card headline should change with it. SwiftShader captures are fine for OG cards (dark scenes quantize well) but the SUBMISSION stills/reel must be regenerated on real GPU hardware before any upload (noted in every doc). When the remote lands: verify prod-crawl.yml + the lighthouse job on real runners; after the owner applies the header rules, flip `WARN_ONLY_HEADERS` to 'false' in prod-crawl.yml.
