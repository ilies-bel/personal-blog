...
## PROGRAM STATUS (updated 2026-07-11 — P14 complete, PROGRAM COMPLETE)
...
- ✅ **P7 mobile access & IA** — WIP checkpoint `ead7f4f`, completed + verified in `d9cc8e1` ("P7 complete"): loader-floor test physics, manifesto-arc stability, 404 baseline, full gate run green.
- ✅ **P8 accessibility (WCAG 2.2 AA)** — committed `7192d09`. Contrast-tested tokens (test/contrast.test.mjs parses tokens.css, audited PAIRS ledger), bright-beat state scrims (.bh-root[data-zone='bright']), 44px coarse-pointer targets (a11y.css + e2e/targets.spec.ts), sitewide double-ring focus system, forced-colors fallbacks, zoom/text-spacing specs, SPA focus handoff. 192 e2e green.
- ✅ **P9 route-specific art direction** — committed (this phase). `body[data-route]` from BaseLayout + fills-only route tokens (`--route-accent`, `--route-grain` in tokens.css); one physical law per route: Work = sustained orbit ('star' backdrop + ±1.6° 210s rotation drift + held 0.7s entry), Writing = coalescing (two-sided scatter → aligned row entry), Graveyard = cooling residue (desaturated/cold live journey + SVG film grain + drawer-pull reveals), Behind the Build = exposed anatomy (measurement-tick section rules + station indices), About = human scale (warm nebula re-grade + serif kicker), Contact = transmission (new 'transmission' AmbientBackdrop variant on poster-4-dot: static signal rings + motion-gated radial pulse). Typography hygiene: mono caps reserved for real records/state (kickers/shelf labels/eyebrows → quiet plain text; About → serif). Route transitions ≤700ms (warp 1500/750 → 560/380; dive tail 0.7s) pinned by test/route-transitions.test.mjs. RouteEnding.astro closes every section page (Work→Contact, Writing→Work, Graveyard→Writing, BtB→Work, About→Contact, Contact→Work). Supernova crescendo (nova² crest: bloom +0.45, radius +0.16, exposure ×1.28, uFlash superlinear) as the arc's midpoint climax. scripts/shoot-routes-grayscale.mjs grayscale contact sheets reviewed. e2e/routes-identity.spec.ts; 210 e2e green.
- ✅ **P10 performance budgets & truthful telemetry** — committed (this phase). `budgets.json` (gzip budgets adjusted to measured reality, reasoning in its comments field) + `scripts/check-bundle-budgets.mjs` (shared measurement core `scripts/lib/bundle-metrics.mjs`: hero engine graph via dynamic-import specifiers, per-route eager JS via HTML entry points + static-import closure, per-page HTML gzip incl. inlined CSS/grain data-URIs); `scripts/gen-perf-report.mjs` writes committed `src/data/perf-report.json` (+ `--check` staleness gate ±5% in CI) and `/behind-the-build` now renders THAT instead of the hand-entered BUDGETS array (unmeasurable INP/DPR rows deleted; provenance line with date+SHA; field-CWV-pending-RUM prose). Lighthouse CI (`@lhci/cli` + `lighthouserc.cjs`, own ci.yml job): CLS ≤0.02 error, LCP/TBT/byte-weight warn until real rendering hardware (software-GL runners; local run: reading routes perf=100/TBT≈0, home LCP 1.55s lab / TBT 155s = SwiftShader artifact, kept out of the page table, raw in perf-report.json). FPS regression floor: `e2e/perf.spec.ts` counts rAF over 8s on `/?tier=low` at stage 0 vs committed `e2e/perf-baseline.json` (floor = 60% of environment median; regen steps in spec header). FpsMeter benchmark now reports min / 1% low / p50 / p95-frame / max. Desktop nav-before-engine spec added to loader.spec.ts (stable-strong form of "nav usable ≤2.5s": clickable while engine chunks are held at the network edge). Measured: hero graph 232.4 KiB gz, home pre-engine 93.6, reading max 15.6, HTML max 50.5. pnpm note: `tslib` devDep + packageExtension added because lighthouse's `intl-messageformat` misses it under node-linker=hoisted.
- ✅ **P11 security & release discipline** — committed (this phase). `scripts/gen-csp-hashes.mjs` derives sha256 hashes for every inline executable script in the built dist (8 unique: the BaseHead pre-paint env script + Astro's island/ClientRouter runtime scripts; ld+json skipped as data) and generates `docs/SECURITY-HEADERS.md` — the exact Cloudflare Response Header Transform values (CSP incl. hashes, HSTS preload, nosniff, Referrer-Policy, Permissions-Policy) with dashboard paste-in steps; `--check` is CI gate 13 (doc hashes must match dist). The FULL policy (incl. `script-src 'self'` + hashes, `default-src 'self'`) was validated by injecting it into a dist copy and driving all 10 routes + a complete home scroll — zero CSP violations, zero console errors, no eval/new Function/workers/inline handlers in the bundle. BaseHead now ships a defense-in-depth meta-CSP carrying only the meta-legal, build-stable subset (`object-src/base-uri/form-action/style-src/img-src/font-src/media-src/connect-src/upgrade-insecure-requests`); script-src deliberately lives ONLY in the edge header because the hashes only exist post-bundle (reasoning documented in BaseHead + the doc); `style-src 'unsafe-inline'` is the documented inlineStylesheets trade-off. No source maps in dist (verified). `docs/RUNBOOK.md`: gh-exact rollback, incident quick-checks, freeze policy (content T−14 / features T−7 / code T−3), RC checklist + three-clean-RC rule, manual 48h post-launch monitoring (no analytics by design), judged-build archive procedure. `.github/workflows/prod-crawl.yml`: weekly cron + dispatch curl crawl of the live domain (apex 200, http→https, www state, key routes, honest 404, sitemap + every sitemap URL 200, security headers WARN-ONLY until the Cloudflare rules are applied). `docs/ANALYTICS.md` stub (NOT ENABLED): CF Web Analytics snippet + exact CSP additions, web-vitals RUM sketch, privacy-note copy. DEPLOY.md gained the Cloudflare Pages preview staging outline. Knip debt retired: deleted dead exports (config.ts lookOffsetX/lookOffsetY/prefersReducedMotion; constants.ts HUD_SELECTED/HUD_STATE_STORAGE_KEY, EXPLORATION_TRIGGER_AT/_REVEAL_DELAY_MS, BEAT_HOLD/FADE, BeatEdge; scroll.ts fadeInOut; tesseract.glsl.ts beamVert/glowVert/glowFrag — beamFrag kept, it feeds the shader catalog; timeline.ts resolve()+LifecyclePosition/LifecycleState and the STAGE_COUNT duplicate), rules block removed from knip.json → everything error-level, green. 212 e2e green WITH the meta-CSP live.
- ✅ **P12 submission tooling** — committed (this phase). Route-specific OG cards: `scripts/gen-og-cards.mjs` composes a 1200×630 card per route + per post from the REAL pages (Playwright against the prod preview; per-route capture mode — home's live black-hole opening frame after scene:ready+loader-gone, the other routes' own backdrop layer with the prose hidden and the dim-wash neutralised via the capture-figures seam, posts as the calm dark room; title/domain bar rendered in the site's own woff2 faces as a transparent browser overlay, composed OFFLINE with sharp over a gradient scrim — NO new inline script, NO card-template page). Cards live in `public/og/` (committed release assets, 6–197KB after the public optimizer; regenerate per release — RUNBOOK RC checklist step 7, no hash gate on purpose: cards are editorial, any recent engine frame is honest). Wiring: `src/lib/ogCard.ts` (existsSync probe → graceful fallback to og-default) consumed by all 7 pages; posts get frontmatter `ogImage` ?? per-post card ?? default. Gates: `scripts/check-og-cards.mjs` = standing gate 14 (every dist page's og:image resolves same-origin, ≤300KB, exactly 1200×630, twitter:image agrees) + `e2e/og.spec.ts` (served-over-HTTP half). Capture suite: `scripts/shoot-submission.mjs` → scratchpad/submission/ (gitignored; 4 main-image candidates incl. supernova crest + red giant pinned via __bhMorph/__bhFlash, 12 coverage stills incl. framed 390×844 mobile pair + reduced-motion/no-JS editions, 800×600 feed previews, summary table) with the committed shot inventory at `docs/awwwards/captures/manifest.json`. Reel: `scripts/record-reel.mjs` records a scripted 1920×1080 pass (loader boot → six-state descent → Work → Graveyard → Behind the Build incl. a Run-live activation → Contact) to scratchpad/submission/reel-raw.webm as RAW footage + prints ffmpeg trim/convert commands (emits mp4 when ffmpeg exists). Campaign templates in `docs/awwwards/`: submission-narratives (50/150/long skeletons, honest tech bullets pre-filled from perf-report.json), elements-captions (8 non-duplicative Elements mapped to shipped features), press-kit (asset manifest + regeneration procedure), ethical-voting-policy (publishable: no fake accounts/requested scores/paid votes/incentives/swaps/bots; permission-based outreach), developer-award-dossier (criteria→artifact map), submission-dry-run (form checklist + two-dry-runs procedure). CSP hashes untouched (no new inline scripts — verified by gate 13).
- ✅ **P13 editorial & case-study scaffolding** — committed (this phase). /projects reframed as proof-first case studies: `src/components/CaseStudy.astro` renders each collection entry (the per-entry markup + styles extracted from projects.astro; the MDX body arrives via slot) with the restructured problem → decisive choice → outcome sections (fleet/mars bodies reworked under `###` heads — same voice, same facts, "what I'm proud of" panels deleted), then PROOF (the claims ledger: every claim with its backing adjacent — evidenced claims show kind/source/link/date; draftEvidence claims render the hedged statement + the quiet mono marker `evidence being gathered`, e2e-pinned to EXACTLY 2 today so retiring debt breaks the pin on purpose), then DECISIONS & CONSEQUENCES (rejectedPaths/limitations/currentStatus from frontmatter, groups absent when data absent — populated in both entries strictly from what the old prose already said). Writing: posts schema gained `type` enum (investigation/essay/failure-story/build-log, default essay) + `evidence[{label,url?}]`; /writing rows expose type + topics(tags) + date + reading time; the Articles shelf auto-splits by type the moment >1 type exists (derived, no content change needed). memory-leak post = investigation with an Evidence aside (Medium original + sentry-java issue #2496 + fix PR #2505 — only artifacts the article already referenced; profiler captures slot in per INPUTS-NEEDED #7); inspiration essay's stale lifecycle wording fixed ("black hole at the bottom" → opens on the collapse, rewinds to the pale dot — matches the reverse arc in AGENTS.md); Voyager/Luminet attributions verified already present in figcaptions (no tesseract photo ships anywhere — the corridor is a shader, nothing to credit). Graveyard: keywordlens gained the second warning sign its prose supported (generative-AI mainstreaming); /graveyard renders hypothesis (the bet, BEFORE the story) + warning signs / what survived (after it) as mono-caps hairline record rows. About: compact "now" block (role/base/availability/proof, real records from BRAND_ROLE + AVAILABILITY + derived shipped count, links to /contact and /projects) directly under the lead — e2e-pinned inside 2×844px at 390w (measured: fits inside ONE viewport); credits & provenance colophon (Lansana Diomande co-authorship, NASA/JPL + Luminet/CNRS imagery mirroring the essay's figcaptions, the three OFL faces → fonts/LICENSES.md, Astro/three.js/React/Playwright). No page h1 changed → OG cards untouched (gate 14 green). No new inline scripts (gate 13 green). 241 e2e passed + 1 expected skip (the desktop-only fps spec on mobile-chrome) across chromium + mobile-chrome.
- ✅ **P14 full-matrix validation & RC-1** — committed (this phase, final). Degraded-mode sweep completed: `e2e/no-webgl.spec.ts` (NEW — getContext('webgl'/'webgl2') stubbed to null via addInitScript → the IMMEDIATE revealWithoutWebgl path must beat the 8s backstop: body.webgl-unavailable ≤7s, on-brand .webgl-fallback note visible + pointer-transparent, zero canvas mounted, nav clickable off the page, zero console/page errors; plus a slow-network smoke — engine chunks held 15s / everything else crawling at 2s route-delays: server-rendered loader paints immediately, Skip releases the page while the engine is still held at the edge). Full RC run against ONE build of this tree, all clean on the FIRST pass (no fixes, no restart): build 10 pages ✓ / check 0 errors / 89 unit / knip clean; all 8 dist gates + html-validate ✓; full Playwright chromium+mobile-chrome (totals in `docs/rc/RC-1.md`). Visual evidence: `scratchpad/shoot-rc.mjs` scroll-through full-page shots of all 10 routes at 1280+390 (scroll-to-bottom-and-back before capture kills the P13 lazy-load artifact) → scratchpad/rc/, grayscale contact sheets regenerated (scratchpad/p9/), human-reviewed — findings recorded as known cosmetic items in the dossier. **`docs/rc/RC-1.md`** = the dossier (run record, full gate table incl. owner-blocked award gates, degraded-mode matrix, environment caveats, cosmetic items, sign-off checklist, RC-1-of-3 counter). RC procedure codified in `docs/roadmaps/acceptance-gates.md`. Owner-session scripts in `docs/validation/`: screen-reader-session.md (NVDA/VO/TalkBack walkthroughs with expected announcements derived from the actual aria structure), usability-session.md (Work ≤15s / Contact ≤30s tasks, one-handed pass, 7-day recall set for the ≥80% gate), device-matrix.md (real devices + the 5-minute thermal procedure with pass bars), proxy-jury.md (9-person blind protocol, 40/30/20/10 rubric with observable anchors, SOTD-winner calibration). INPUTS-NEEDED #8 now points at these.

## PROGRAM COMPLETE (P1–P14, 2026-07-11)

**What's done** — every code-implementable roadmap item: trust repair (no-JS
static edition, 404, JSON-LD), the e2e+CI foundation (15 standing gates, 5-project
Playwright matrix), motion centralization, WebGL governance (≤2 contexts, zero
three.js on reading routes), the image pipeline, evidence-enforcing content
collections, mobile access + IA + Contact + honest loader, WCAG 2.2 AA, per-route
art direction with ≤700ms transitions, enforced performance budgets + generated
telemetry, CSP/security headers + release runbook, submission tooling (OG cards,
capture suite, campaign templates), proof-first editorial scaffolding, and the
full-matrix RC-1 with dossier + owner validation scripts. All on `soty/program`,
one phase per commit, every commit green through the standing gates.

**What awaits owner inputs** (`docs/INPUTS-NEEDED.md`): #1 remote/CI (proves the
full browser matrix + Lighthouse on real hardware; promotes gate 11 to error;
enables prod crawl + real RC-2/RC-3), #2–#5+#7 evidence/content (de-hedge the two
draftEvidence claims, third flagship, testimonials, specimens/articles), #6
availability truth, #8 human validation (scripts ready in `docs/validation/`),
#9 Cloudflare header paste (then flip WARN_ONLY_HEADERS in prod-crawl.yml),
#10 Awwwards campaign execution. Submission stills/reel: regenerate on real GPU.

**How to cut RC-2/RC-3**: follow the RC procedure in
`docs/roadmaps/acceptance-gates.md` (run on real CI once #1 lands: all five
Playwright projects + the Lighthouse job), record `docs/rc/RC-2.md` /`RC-3.md`
from RC-1's structure, tag `v2.0.0-rc<n>` per `docs/RUNBOOK.md`. Three
consecutive clean runs with zero fixes in between → cut the submission tag.

### Resume point (for any future session)
1. The program is complete; nothing is in flight. Owner-side work is enumerated in `docs/INPUTS-NEEDED.md`; the release path is RUNBOOK freeze schedule → RC-2/RC-3 → tag.
2. Notes carried from P13/P14: the OG cards bake page h1 copy into `scripts/gen-og-cards.mjs`'s CARDS table (title strings) — any future editorial pass that rewrites a page h1 must update the card title there and regenerate `public/og/` in the same commit (gate 14 won't catch copy drift, only broken/missized cards; P13 changed no h1s). SwiftShader captures are fine for OG cards (dark scenes quantize well) but the SUBMISSION stills/reel must be regenerated on real GPU hardware before any upload (noted in every doc). When the remote lands: verify prod-crawl.yml + the lighthouse job on real runners; after the owner applies the header rules, flip `WARN_ONLY_HEADERS` to 'false' in prod-crawl.yml. P13 content seams for the owner's inputs: de-hedging Fleet/Mars claims = swap `evidence.type: none` for the real record + drop `draftEvidence` (the claims ledger renders it automatically; remember the e2e marker-count pin in e2e/content.spec.ts); new posts should author `type` deliberately (shelving activates by itself); profiler captures for the memory-leak post append to its frontmatter `evidence` list.
3. Carried P9 notes: the marker DIVE (home → article) still runs its authored 1.5s cinematic (deliberately exempt as a hero set-piece; revisit if the jury reads it as a route transition); the graveyard/behind-the-build live ArticleScene boots slowly on software renderers (scene:ready can exceed 45s; the `load` event can stall past 30s — capture scripts navigate on domcontentloaded) — keep capture scripts' long timeouts.
